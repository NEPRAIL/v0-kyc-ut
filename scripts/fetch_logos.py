...
CLEARBIT = "https://logo.clearbit.com/{domain}"  # usually PNG
...
def read_catalog(path: Path):
    items = []
    if not path.exists():
        sys.exit(f"Missing {path}. Create it and paste your catalog.")
...
def fetch_wikimedia_svg(query: str):
    params = {
        ...
        "prop":"imageinfo","generator":"search",
        "gsrsearch": f'filetype:bitmap|drawing {query} logo',
        "gsrlimit":"5","iiprop":"url|mime"
    }
