...
import { users, sessions } from "@/db/schema"; // adjust paths
...
export async function POST(req: Request) {
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json({ error: "Invalid input", code: "VALIDATION_ERROR", issues: parsed.error.issues }, { status: 400 });
    }
